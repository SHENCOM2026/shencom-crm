...
/* ───── Venta Form trigger inside LeadModal ───── */
function VentaFormTrigger({ lead }) {
    ...
    <>
      <div className="border-t pt-4">
        <button
          ...
          className="w-full flex items-center justify-center gap-2 py-3 border-2 border-claro-red text-claro-red rounded-xl text-sm font-semibold hover:bg-red-50 transition-colors"
        >
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          Formulario de Ingreso de Venta
        </button>
        <p className="text-xs text-gray-400 text-center mt-1.5">Datos del cliente, forma de pago, líneas y documentos adjuntos</p>
      </div>
      <VentaForm isOpen={open} onClose={() => setOpen(false)} lead={lead} />
    </>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          {/* Venta form — only when editing existing lead */}
          {lead?.id && <VentaFormTrigger lead={lead} />}
